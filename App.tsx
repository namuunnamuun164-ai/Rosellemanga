...
export default function App() {
  ...
  // ЗАСВАР #201: staff-ийн сэтгэгдлийн мэдэгдэл
  ...
  // ЗАСВАР #201: staff-д уншигчдын сэтгэгдлийн мэдэгдэл — анхны 50-г татаад,
  // цаашид шинэ мэдэгдэл орох бүрд realtime-аар шууд авна (RLS-ээр зөвхөн
  // өөрийн мэдэгдэл ирнэ). Staff биш / гараагүй бол хоослоно.
  useEffect(() => {
    if (!currentUser) { setNotifications([]); return; }
    const sel = 'id, is_read, created_at, comment_id, chapter_id, manga_id, actor:actor_id(name), manga:manga_id(id, title), chapter:chapter_id(chapter_number), comment:comment_id(content, parent_id)';
    let cancelled = false;
    supabase.from('notifications').select(sel)
      .eq('recipient_id', currentUser.id)
      .order('created_at', { ascending: false })
      .limit(50)
      .then(({ data }) => { if (!cancelled && data) setNotifications(data); });

    const channel = supabase.channel(`notif_${currentUser.id}`)
      .on('postgres_changes',
        { event: 'INSERT', schema: 'public', table: 'notifications', filter: `recipient_id=eq.${currentUser.id}` },
        async (payload) => {
          // realtime payload дээр join (actor/manga/chapter) байхгүй тул тухайн
          // мөрийг дэлгэрэнгүйгээр нь нэг удаа дахин татна.
          const { data } = await supabase.from('notifications').select(sel).eq('id', payload.new.id).maybeSingle();
          if (data) {
            setNotifications(prev => prev.some(n => n.id === data.id) ? prev : [data, ...prev].slice(0, 50));
            notify('🔔 Шинэ мэдэгдэл');
          }
        })
      .subscribe();
  ...
  const markNotifsRead = useCallback(async (ids) => {
  ...
  const openNotif = useCallback((n) => {
    markNotifsRead([n.id]);
    setNotifOpen(false);
    const mangaId = n.manga?.id ?? n.manga_id;
    const m = dbMangas.find(x => x.id === mangaId);
    if (m) { setPreviousPage(page); setSelected(m); setPage('detail'); }
    else notify('Холбогдох манга олдсонгүй (устсан байж магадгүй).');
  }, [dbMangas, page, markNotifsRead, notify]);
  ...
  const postComment = async (parentId = null, textOverride = null) => {
    ...
    const { error } = await supabase.from('comments').insert({
      ...
      // ЗАСВАР #201: manga_id-г ч хадгалж, сэтгэгдлийн өгөгдлийг бүрэн болгов
      // (мэдэгдлийн навигаци/гарчиг chapter-аас нэмж хайхгүйгээр шууд ажиллана).
      manga_id: selected?.id ?? null,
          ...
          <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexShrink: 0 }}>
            ...
            {currentUser ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                {/* ЗАСВАР #205: мэдэгдлийн хонх — бүх нэвтэрсэн хэрэглэгчид (staff: бүх
                    сэтгэгдэл; энгийн хэрэглэгч: зөвхөн өөрийн сэтгэгдэлд ирсэн зүрх/хариулт). */}
                {currentUser && (
                  <div style={{ position: 'relative' }}>
                    <button onClick={() => setNotifOpen(o => !o)} title="Мэдэгдэл"
                      style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', width: 38, height: 38, background: 'rgba(255,255,255,0.06)', border: '1px solid #2a2a2a', borderRadius: 8, cursor: 'pointer', color: '#ddd' }}>
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>
                      {unreadNotifCount > 0 && (
                        <span style={{ position: 'absolute', top: -4, right: -4, minWidth: 18, height: 18, padding: '0 5px', boxSizing: 'border-box', background: '#8B0000', color: '#fff', fontSize: 11, fontWeight: 700, borderRadius: 9, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid #0a0a0a' }}>
                    ...
                    {notifOpen && (
                      <>
                        ...
                        <div style={{ position: 'absolute', top: '120%', right: 0, width: 340, maxWidth: '92vw', maxHeight: 440, overflowY: 'auto', background: '#111', border: '1px solid #222', borderRadius: 16, padding: '0.75rem', zIndex: 291, boxShadow: '0 12px 32px rgba(0,0,0,0.5)' }}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '4px 8px 10px' }}>
                            <span style={{ fontWeight: 700, fontSize: 14 }}>Мэдэгдэл</span>
                            {notifications.some(n => !n.is_read) && (
                              <button onClick={() => markNotifsRead(notifications.filter(n => !n.is_read).map(n => n.id))}
                                style={{ background: 'none', border: 'none', color: '#8B0000', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>Бүгдийг уншсан</button>
                            )}
                          </div>
                          {notifications.length === 0 ? (
                            <div style={{ color: '#666', textAlign: 'center', padding: '2rem 0', fontSize: 13 }}>Мэдэгдэл алга байна</div>
                          ) : notifications.map(n => (
                            <div key={n.id} onClick={() => openNotif(n)}
                              style={{ display: 'flex', gap: 10, padding: '10px 8px', borderRadius: 10, cursor: 'pointer', background: n.is_read ? 'transparent' : 'rgba(139,0,0,0.08)', alignItems: 'flex-start' }}>
                              <div style={{ width: 8, height: 8, borderRadius: '50%', background: n.is_read ? 'transparent' : '#8B0000', marginTop: 6, flexShrink: 0 }} />
                              <div style={{ minWidth: 0, flex: 1 }}>
                                <div style={{ fontSize: 13, lineHeight: 1.35 }}>
                                  <b>{n.actor?.name || 'Хэрэглэгч'}</b>
                                  {n.type === 'like' ? ' таны сэтгэгдэлд ❤️ дарлаа'
                                    : n.type === 'reply' ? ' таны сэтгэгдэлд хариуллаа'
                                    : (n.comment?.parent_id ? ' хариулт бичлээ' : ' сэтгэгдэл бичлээ')}
                                  {n.manga?.title ? ` — ${n.manga.title}` : ''}
                                  {n.chapter?.chapter_number != null ? ` (${n.chapter.chapter_number}-р бүлэг)` : ''}
                                </div>
                                {n.comment?.content ? (
                                  <div style={{ color: '#999', fontSize: 12, marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{n.comment.content}</div>
                                ) : null}
                                <div style={{ color: '#666', fontSize: 11, marginTop: 3 }}>{formatNumericDate(n.created_at)}</div>
                              </div>
                            </div>
                          ))}
                        </div>
            ...
            <div>
              ...
              <div style={{ background: '#111', borderRadius: 12, padding: '1.5rem', border: '1px solid #1e1e1e', maxWidth: 480 }}>
                ...
                <div style={{ marginBottom: 12 }}>
                  ...
                  {/* ЗАСВАР #202: горим сонголтыг авав — урт зургийг ҮРГЭЛЖ автоматаар
                      8000px-ээр хэсэглэнэ. Зүгээр л урт зургаа бүтнээр нь хаяхад л болно. */}
                  <div style={{ fontSize: 10, color: '#f5a623', marginBottom: 8 }}>Урт зураг автоматаар 8000px-ээр олон хуудас болж хуваагдана (өргөн хэвээрээ үлдэнэ).</div>
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    // ЗАСВАР #202: урт зургийг ҮРГЭЛЖ (горим сонголтгүйгээр) 8000px-ээр
                    // автоматаар хэсэглэнэ — уншигч урт зургаа зүгээр л хаяхад л болно.
                    let filesToUpload = chapterFiles;
                    {
                      ...
                      try {
                        for (const f of chapterFiles) {
                          ...
                          const parts = await splitTallImageFile(f, 8000);
                          expanded.push(...parts);
                        }
                      } catch (e) {
                        notify('Алдаа: ' + e.message);
          ...
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999, padding: 16 }}>
            <div style={{ width: 520, maxWidth: '100%', maxHeight: '90vh', overflowY: 'auto', background: '#111', border: '1px solid #222', borderRadius: 20, padding: '2rem', position: 'relative', boxSizing: 'border-box' }}>
              ...
              <button disabled={editChapterSaving} onClick={async () => {
                ...
                let uploadFailed = false;
                // ЗАСВАР #200: засах замд ч мөн (шинэ бүлэг оруулахтай адил) 4000px-ээс
                // урт зургийг upload-аас ӨМНӨ автоматаар хэсэглэнэ. Өмнө нь энд огт
                // хуваадаггүй байсан тул засах цэсээр нэмсэн урт (жишээ нь 60000px)
                // зураг бүтнээрээ R2-д ороод, утсан дээр decode хийж чадахгүй болж
                // "зурганд алдаа" гардаг байв. splitTallImageFile нь 4000px-ээс богино
                // зургийг үл хөндөж, өөрийг нь буцаадаг тул ердийн зурагт нөлөөгүй.
                for (const file of editChapterNewFiles) {
                  try {
                    // eslint-disable-next-line no-await-in-loop
                    const parts = await splitTallImageFile(file, 8000);
                    for (const part of parts) {
                      const ext = part.name.split('.').pop();
                      // ЗАСВАР #194: crypto.randomUUID() — дараалсан нэр таамаглагдахаас сэргийлнэ
                      // eslint-disable-next-line no-await-in-loop
